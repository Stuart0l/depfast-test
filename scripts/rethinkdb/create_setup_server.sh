...
function az_vm_create {
  ...
  do
    az vm create --name rethinkdb"$namePrefix"-"$i" --resource-group DepFast --subscription 'Microsoft Azure Sponsorship 2' --zone 1 --image debian --os-disk-size-gb 500 --storage-sku Premium_LRS --data-disk-sizes-gb 500 --size Standard_D4s_v3 --admin-username riteshsinha --ssh-key-values ~/.ssh/id_rsa.pub --accelerated-networking true
...
ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa $clientPublicIP << EOF_2
	...
	cd ~/YCSB
	:
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 1 azure noslow disk swapoff 3

	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 1 azure follower disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 2 azure follower disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 3 azure follower disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 4 azure follower disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 5 azure follower disk swapoff 3

	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 1 azure leader disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 2 azure leader disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 3 azure leader disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 4 azure leader disk swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 5 azure leader disk swapoff 3
EOF_2
}

function run_memory_experiment {
ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa $clientPublicIP << EOF_4
	# Run all the experiments one by one
	cd ~/YCSB
	:
...
ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa $clientPublicIP << EOF_3
	...
	cd ~/YCSB
	:
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 1 azure noslow memory swapoff 3

	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 1 azure follower memory swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 2 azure follower memory swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 5 azure follower memory swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 6 azure follower memory swapon 3

	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 1 azure noslow memory swapon 3

	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 1 azure leader memory swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 2 azure leader memory swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 5 azure leader memory swapoff 3
	./start_experiment.sh $iterations workloads/$workload $ycsbruntime 6 azure leader memory swapon 3
EOF_3
}
