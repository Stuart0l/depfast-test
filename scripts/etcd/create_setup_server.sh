...
function az_vm_create {
  # Create client VM
  az vm create --name etcd-"$namePrefix"-client --resource-group $resource --subscription "$subscription" --zone 1 --image debian --os-disk-size-gb 64 --storage-sku Standard_LRS  --size Standard_D4s_v3 --admin-username $username --ssh-key-values ~/.ssh/id_rsa.pub --accelerated-networking true
  ...
  ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa $clientPublicIP -l $username 'ssh-keygen -t rsa -f ~/.ssh/id_rsa -q -P "" <<<y 2>&1 >/dev/null '
  # Scp the client id_rsa.pub to local directory
  scp -i ~/.ssh/id_rsa $username@$clientPublicIP:~/.ssh/id_rsa.pub ./client_rsa.pub

  # Create servers with both local ssh key and client VM ssh key
  for (( i=1; i<=noOfServers; i++ ))
  do
    az vm create --name etcd-"$namePrefix"-"$i" --resource-group $resource --subscription "$subscription" --zone 1 --image debian --os-disk-size-gb 64 --storage-sku Premium_LRS --data-disk-sizes-gb 64 --size Standard_D4s_v3 --admin-username $username --ssh-key-values ~/.ssh/id_rsa.pub ./client_rsa.pub --accelerated-networking true
  done
}
...
function setup_servers {
  ...
  do
	...
	# Install etcd
	ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa $username@${serverNameIPMap[$key]} "sudo sh -c './install_etcd.sh'"
...
function setup_client {
	...
	scp -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa -r ./* $username@$clientPublicIP:~/

	scp -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa -r install_etcd.sh $username@$clientPublicIP:~/
	# Install etcd
	...
	ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa $username@$clientPublicIP "/bin/bash --login -c 'go version'"
