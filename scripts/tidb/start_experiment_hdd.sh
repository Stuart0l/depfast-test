...
function start_servers {  
  if [ "$host" == "gcp" ]; then
    ...
    az vm start --resource-group DepFast3 --name "$pdname"
    az vm start --resource-group DepFast3 --name "$s1name"
    az vm start --resource-group DepFast3 --name "$s2name"
    az vm start --resource-group DepFast3 --name "$s3name"
  else
...
function stop_servers {
  if [ "$host" == "gcp" ]; then
    ...
    az vm deallocate --resource-group DepFast3 --name "$s1name"
    az vm deallocate --resource-group DepFast3 --name "$s2name"
    az vm deallocate --resource-group DepFast3 --name "$s3name"
    az vm deallocate --resource-group DepFast3 --name "$pdname"
  else
    echo "Not implemented error"
    exit 1
  fi
