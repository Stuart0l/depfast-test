...
function az_vm_create {
  ...
  clientConfig=$(az vm list-ip-addresses --subscription "Last Chance" --name tidb"$namePrefix"_client --query '[0].{name:virtualMachine.name, privateip:virtualMachine.network.privateIpAddresses[0], publicip:virtualMachine.network.publicIpAddresses[0].ipAddress}' -o json)
...
function write_config {
	rm -f config.json
	az vm list-ip-addresses --subscription "Last Chance" --ids $(az vm list --query "[].id" --subscription "Last Chance" --resource-group DepFast3 -o tsv | grep $serverRegex) --query '[].{name:virtualMachine.name, privateip:virtualMachine.network.privateIpAddresses[0], publicip:virtualMachine.network.publicIpAddresses[0].ipAddress}' -o json > config.json
}
...
function deallocate_vms {
	ns=$(az vm list --query "[].id" --subscription "Last Chance" --resource-group DepFast3 -o tsv | grep $serverRegex | wc -l)
	if [[ $ns -le 5 ]]
	then
		az vm deallocate --subscription "Last Chance" --ids $(
			az vm list --query "[].id" --subscription "Last Chance" --resource-group DepFast3 -o tsv | grep $serverRegex
		)
	else
